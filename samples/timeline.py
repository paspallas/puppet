...
class TimeLine(QWidget):
    ...
    def __init__(self, duration, length, parent):
        ...
        for i in range(0, 5):
            self.videoSamples.append(VideoSample(i * 10))
    ...
    def paintEvent(self, event):
        ...
        while w <= self.width():
            qp.drawText(
                w - 50, 0, 100, 100, Qt.AlignHCenter, self.get_time_string(w * scale)
            )
            w += 100
        # Draw down line
        ...
        for sample in self.videoSamples:
            ...
            if sample.picture is not None:
                if sample.picture.size().width() < sample.duration / scale:
                    path = QPainterPath()
                    path.addRoundedRect(
                        QRectF(t / scale, 52.5, sample.picture.size().width(), 45),
                        10,
                        10,
                    )
                    qp.setClipPath(path)
                    qp.drawPixmap(
                        QRect(t / scale, 52.5, sample.picture.size().width(), 45),
                        sample.picture,
                    )
                else:
                    path = QPainterPath()
                    path.addRoundedRect(
                        QRectF(t / scale, 52.5, sample.duration / scale, 45), 10, 10
                    )
                    qp.setClipPath(path)
                    pic = sample.picture.copy(0, 0, sample.duration / scale, 45)
                    qp.drawPixmap(
                        QRect(t / scale, 52.5, sample.duration / scale, 45), pic
                    )
            t += sample.duration
    ...
    def wheelEvent(self, e) -> None:
        self._scale += e.angleDelta().y() / 120
...
if __name__ == "__main__":
    ...
    timeline = TimeLine(360, 600, None)
    timeline.show()
